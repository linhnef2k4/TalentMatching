...
/* ─────────────────────────────────────────────
   CONSTANTS
───────────────────────────────────────────────*/
const WS_BASE = 'ws://160.191.214.94:8080';
const WS_URL = `${WS_BASE}/ws/websocket`;
...
const ChatApp = () => {
    ...
    // Performance Refs (O(1) Check, Debounce, Throttle)
    const messagesMapRef = useRef(new Map());
    const reconnectAttemptsRef = useRef(0);
    const fetchConvThrottleRef = useRef(0);
        ...
        const boot = () => {
            const client = new Client({
                ...
                onConnect: () => {
                    ...
                    reconnectAttemptsRef.current = 0; // Reset count khi connect thành công
                    ...
                    client.subscribe(`/user/${user.email}/queue/messages`, (frame) => {
                        ...
                        setMessages((prev) => {
                            ...
                            const optIndex = prev.findIndex(
                                (p) => p.optimistic && p.content.trim() === msg.content.trim(),
                            );
                            if (optIndex !== -1) {
                                const newMsgs = [...prev];
                        ...
                        setConversations((prev) => {
                            ...
                            if (Date.now() - fetchConvThrottleRef.current > 5000) {
                                fetchConvThrottleRef.current = Date.now();
                                fetchConversations();
                            }
                            return prev;
                        });
            ...
            {wsStatus === 'disconnected' && (
                <div className="mb-3 bg-red-50 text-red-600 px-4 py-2.5 rounded-xl flex items-center justify-between font-medium text-sm border border-red-200 w-full shrink-0 shadow-sm animate-in fade-in slide-in-from-top-2">
                    <div className="flex items-center gap-2">
                        <WifiOff size={18} /> Mất kết nối tới máy chủ. Tin nhắn sẽ không được gửi. Đang tự động kết nối
                        lại...
                    </div>
                    <button
                ...
                <aside className="w-1/3 border-r border-gray-200 flex flex-col bg-gray-50/50 min-w-[280px]">
                    <div className="p-4 border-b border-gray-200 shrink-0">
                        <div className="flex items-center justify-between mb-4">
                            <h2 className="text-xl font-bold text-gray-800">Tin Nhắn Ứng Viên</h2>
                            <span
                        ...
                        <div className="relative">
                            ...
                            <input
                                ...
                                className="w-full bg-white border border-gray-300 rounded-xl pl-10 pr-4 py-2 text-sm outline-none focus:border-indigo-500 transition"
                            />
                        </div>
                    </div>
                    ...
                    <div className="flex-1 overflow-y-auto">
                        {isLoadingChats ? (
                            ...
                            <>
                                {activeContact?.isNew && (
                                    <div className="flex items-center gap-3 p-4 cursor-pointer border-b border-gray-100 bg-indigo-50 border-l-4 border-l-indigo-600">
                                        <Avatar
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <p className="text-sm truncate text-indigo-600 font-medium">Liên hệ mới</p>
                                        </div>
                                    </div>
                                )}
                                {filtered.map((conv) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                >
                                                    ...
                                                    <div
                                                        className={`text-[10px] text-gray-400 mt-1.5 px-1 font-medium ${
                                                            isMe ? 'text-right' : 'text-left'
                                                        }`}
                                                    >
                                                        {fmt(item.createdAt)}
                                                        {item.optimistic && ' · Đang gửi...'}
                                                    </div>
                                                </div>
                                            </div>
                                        );
                            ...
                            <div className="flex-shrink-0 px-4 py-3 border-t border-gray-200 bg-white relative">
                                ...
                                >
                                    ...
                                    <textarea
                                        ...
                                        className="flex-1 bg-transparent py-3 px-1 text-[14px] leading-tight text-gray-700 placeholder-gray-400 outline-none disabled:cursor-not-allowed resize-none max-h-32 min-h-[44px] [&::-webkit-scrollbar]:w-[4px] [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-slate-300 [&::-webkit-scrollbar-thumb]:rounded-[4px] my-auto"
                                        rows="1"
                                    />
                                    <button
                                        type="submit"
                                        disabled={!inputMessage.trim() || wsStatus !== 'connected' || isUploading}
                                        className="w-[44px] h-[44px] shrink-0 bg-indigo-600 rounded-full flex items-center justify-center text-white hover:bg-indigo-700 active:scale-95 transition-all disabled:bg-slate-200 disabled:cursor-not-allowed"
                                    >
                                        {isUploading ? (
